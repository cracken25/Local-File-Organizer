...
def compute_operations(data_list, new_path, renamed_files, processed_files):
    ...
    for data in data_list:
        ...
        # Handle both old format (foldername) and new format (workspace/subpath)
        if 'workspace' in data:
            # New taxonomy-based format
            workspace = data['workspace']
            subpath = data.get('subpath', '')
            
            # Build directory path: output/workspace/subpath
            if subpath:
                dir_path = os.path.join(new_path, workspace, subpath)
            else:
                dir_path = os.path.join(new_path, workspace)
            
            # Use the generated filename with extension
            new_file_name = data['filename'] + os.path.splitext(file_path)[1]
            confidence = data.get('confidence', 0)
        else:
            # Old format for backwards compatibility (date/type sorting)
        ...
        while new_file_path in renamed_files:
            new_file_name = f"{base_filename}_{counter}" + os.path.splitext(file_path)[1]
            new_file_path = os.path.join(dir_path, new_file_name)
            counter += 1
        ...
        # Add workspace/subpath info for display
        if 'workspace' in data:
            operation['workspace'] = data['workspace']
            operation['subpath'] = data.get('subpath', '')
